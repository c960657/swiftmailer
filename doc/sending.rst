...
* ``Swift_DirectSmtpTransport``: Sends messages over SMTP directly to the
  ...
  feedback on delivery errors; All recipient addresses for each message must
  have the same domain part; Fails if remote mail server is temporarily down.
...
One significant limitation with the Direct SMTP Transport is that all recipient
addresses (``To:``, ``Cc:`` and ``Bcc:``) of a given message must have same
domain part.
